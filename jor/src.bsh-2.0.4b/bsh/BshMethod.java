...
public class BshMethod implements java.io.Serializable {
    ...
    */
    ...
    BshMethod(BSHMethodDeclaration method, NameSpace declaringNameSpace, Modifiers modifiers) {
        this(method.name, method.returnType, method.paramsNode.getParamNames(),
            method.paramsNode.paramTypes, method.blockNode, declaringNameSpace, modifiers);
    }

    BshMethod(String name, Class returnType, String[] paramNames, Class[] paramTypes,
        BSHBlock methodBody, NameSpace declaringNameSpace, Modifiers modifiers) {
        this.name = name;
    ...
    */
    BshMethod(Method method, Object object) {
        this(method.getName(), method.getReturnType(), null /*paramNames*/,
            method.getParameterTypes(), null /*method.block*/, null /*declaringNameSpace*/,
            null /*modifiers*/);
    ...
    */
    public Object invoke(Object[] argValues, Interpreter interpreter)
        throws EvalError {
    ...
    */
    public Object invoke(Object[] argValues, Interpreter interpreter, CallStack callstack,
    ...
    */
    Object invoke(Object[] argValues, Interpreter interpreter, CallStack callstack,
        ...
        if (argValues != null) {
            for (int i = 0; i < argValues.length; i++)
                if (argValues[i] == null) {
                    argValues[i] = Primitive.NULL;
                }
        }

        //	throw new Error("HERE!");
        if (javaMethod != null) {
            //			if("sum".equals(this.name))
            //				System.out.println();
            try {
                ...
                throw new TargetError("Exception invoking imported object method.", e2, callerInfo,
                    callstack, true /*isNative*/);
            }
        }
    ...
    private Object invokeImpl(Object[] argValues, Interpreter interpreter, CallStack callstack,
        SimpleNode callerInfo, boolean overrideNameSpace)
        throws EvalError {
        ...
        if (argValues.length != numArgs) {
            ...
            */
            throw new EvalError("Wrong number of arguments for local method: " + name, callerInfo,
                callstack);
        }
        ...
        for (int i = 0; i < numArgs; i++) {
            ...
            if (paramTypes[i] != null) {
                try {
                    ...
                    throw new EvalError("Invalid argument: " + "`" + paramNames[i] + "'" +
                        " for method: " + name + " : " + e.getMessage(), callerInfo, callstack);
                }

                try {
                    localNameSpace.setTypedVariable(paramNames[i], paramTypes[i], argValues[i],
                        null /*modifiers*/);
            ...
            // Set untyped variable
            else // untyped param
             {
                ...
                if (argValues[i] == Primitive.VOID) {
                    throw new EvalError("Undefined variable or class name, parameter: " +
                        paramNames[i] + " to method: " + name, callerInfo, callstack);
                } else {
                    try {
                        localNameSpace.setLocalVariable(paramNames[i], argValues[i],
        ...
        if (ret instanceof ReturnControl) {
            ...
            if (retControl.kind == retControl.RETURN) {
                ...
                throw new EvalError("'continue' or 'break' in method body", retControl.returnPoint,
                    returnStack);
            }
            ...
            if ((returnType == Void.TYPE) && (ret != Primitive.VOID)) {
                throw new EvalError("Cannot return value from void method", retControl.returnPoint,
                    returnStack);
            }
        }
        ...
        if (returnType != null) {
            ...
            try {
                ...
                throw e.toEvalError("Incorrect type returned from method: " + name +
