...
public class Interpreter implements Runnable, ConsoleInterface, Serializable {
    ...
    */
    ...
    static XPathProcessor xpathProcessor;
    ...
    */
    ...
    protected boolean evalOnly; // Interpreter has a user, print prompts, etc.
    ...
    private Interpreter localInstance;
    private Interpreter root;
    ...
    */
    ...
    private void initRootSystemObject2() {
    ...
    */
            ...
            try {
                ...
                if (result instanceof Class) {
                    try {
                        ...
                        System.err.println("Class: " + result + " main method threw exception:" +
                            o);
                    }
                }
    ...
    public static void invokeMain(Class clas, String[] args)
        throws Exception {
        Method main = Reflect.resolveJavaMethod(null /*BshClassManager*/, clas, "main",
    ...
    */
     ...
     * @param in DOCUMENT ME!
     * @param nameSpace DOCUMENT ME!
     * @param sourceFileInfo DOCUMENT ME!
     *
     * @return DOCUMENT ME!
     *
     * @throws EvalError DOCUMENT ME!
     */
    public Object eval(Reader in, NameSpace nameSpace,
        String sourceFileInfo /*, CallStack callstack */)
        throws EvalError {
        Object retVal = null;
        ...
        /*
                ...
                this interpreter.
        */
        localInstance = new Interpreter(in, out, err, false, nameSpace, this, sourceFileInfo);

        CallStack callstack = new CallStack(nameSpace);
        SimpleNode node = (SimpleNode) cacher.get(sourceFileInfo);

        if (node != null) {
            //System.out.println(sourceFileInfo);
            retVal = eval(node, nameSpace, callstack, localInstance);
        } else {
            retVal = eval(nameSpace, sourceFileInfo, callstack, localInstance);
        }

        return Primitive.unwrap(retVal);
    }

    private Object eval(NameSpace nameSpace, String sourceFileInfo, CallStack callstack,
        Interpreter localInterpreter) throws ParseException, EvalError {
        Object retVal = null;
        boolean eof = false;
        ...
        while (!eof) {
            ...
            try {
                ...
                */
                ...
                e.printStackTrace();
                throw new EvalError("Sourced file: " + sourceFileInfo + " internal Error: " +
                ...
                // }
                throw new EvalError("Sourced file: " + sourceFileInfo + " unknown error: " +
                    e.getMessage(), node, callstack);
            } catch (TokenMgrError e) {
                e.printStackTrace();
                throw new EvalError("Sourced file: " + sourceFileInfo + " Token Parsing Error: " +
                    e.getMessage(), node, callstack);
            } finally {
        ...
        //  if ((lastNode != null) && (count == 1)) {
        cacher.put(sourceFileInfo, firstNode);
     ...
     * @throws EvalError DOCUMENT ME!
     */
    public Object eval(SimpleNode node) throws ParseException, EvalError {
        NameSpace ns = this.getNameSpace();
        CallStack callstack = new CallStack(ns);
        Object result = eval(node, ns, callstack,
                (this.localInstance == null) ? this : this.localInstance);

        return Primitive.unwrap(result);
    }

    private Object eval(SimpleNode node, NameSpace nameSpace, CallStack callstack,
        Interpreter localInterpreter) throws ParseException, EvalError {
        Object retVal = null;
    ...
    */
    public Object eval(String statements, NameSpace nameSpace)
        ...
        return eval(new StringReader(s), nameSpace, statements);
    }
    ...
    */
    public Object get(String name) throws EvalError {
        try {
            ...
            if (ret instanceof ValueIfClosed) {
                ValueIfClosed vic = (ValueIfClosed) ret;

                if (vic instanceof InterpreterAware) {
                    ((InterpreterAware) vic).setInterpreter(this);
                }
    ...
    */
    public Object get2(String name) throws EvalError {
        try {
            Object ret = globalNameSpace.get(name, this);
        ...
        */
        ...
        try {
            ...
            if (lhs.type != LHS.VARIABLE) {
                throw new EvalError("Can't unset, not a variable: " + name, SimpleNode.JAVACODE,
                    new CallStack());
            }
    ...
    */
     ...
     * DOCUMENT ME!
     *
     * @param xpathProcessor DOCUMENT ME!
     */
    public static void setXPathProcessor(XPathProcessor xpathProcessor) {
        Interpreter.xpathProcessor = xpathProcessor;
    }
    ...
    public SimpleNode parse(String expr) {
        ...
        String s = (expr.endsWith(";") ? expr : (expr + ";"));
        ...
        try {
            ...
            if (parser.jjtree.nodeArity() > 0) {
                result = (SimpleNode) parser.jjtree.rootNode();
