...
class LHS implements ParserConstants, java.io.Serializable {
    ...
    */
    ...
    /**
            Identifiers for the various types of LHS.
    */
    static final int CLINET_PROPERTY = 5;
    private static final int JATOOLS_PROPERTY = 0;
    NameSpace nameSpace;
    ...
    */
    ...
    Object property LHS Constructor.
    */
    LHS(PropertySetter object, String propName) {
        if (object == null) {
            throw new NullPointerException("constructed empty LHS");
        }

        type = CLINET_PROPERTY;
        this.object = object;
        this.propName = propName;
    }
    ...
    public Object assign(Object val, boolean strictJava)
        ...
        if (type == VARIABLE) {
            ...
            if (localVar) {
                ...
                Variable existing = nameSpace.getVariableImpl(varName, true);

                if (existing != null) {
                    try {
                        existing.setValue(val, Variable.ASSIGNMENT);
                    } catch (UtilEvalError e) {
                        throw new UtilEvalError("Variable assignment: " + varName + ": " +
                            e.getMessage());
                    }
                } else {
                    nameSpace.getRoot().setLocalVariable(varName, val, strictJava);
                }
            }
        } else if (type == FIELD) {
            try {
                ...
                throw new UtilEvalError("Argument type mismatch. " +
                    ((val == null) ? "null" : type) + " not assignable to field " +
                    field.getName());
            }
        ...
        } else if (type == CLINET_PROPERTY) {
            PropertySetter setter = (PropertySetter) object;
            setter.setProperty(propName, val);
        }
        else if (type == INDEX) {
            try {
